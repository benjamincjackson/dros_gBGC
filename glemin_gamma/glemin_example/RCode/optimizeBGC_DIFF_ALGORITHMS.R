...
optimizeBGCDiffAl <- function(dataNeutral,dataWS,dataSW,dataGC,
                              optimAlgorithm, # BEN: I have added this argument to use to change the methods used by the optim() function
  ...
  # Optimization of model 0
	...
	ml0 <- optim(init0,lnL0,SFSneutral=dataNeutral,SFSws=dataWS,SFSsw=dataSW,GC=dataGC,CatInf=BoundInf,CatSup=BoundSup,lower=inf0,upper=sup0,method=optimAlgorithm,control=list(maxit=MAXIT,factr=FACTR,lmm=LMM))
	warning <- FALSE
	# Output of model 0
  ...
  if(PrintInit){
	...
	ml1 <- optim(init1,lnL1,SFSneutral=dataNeutral,SFSws=dataWS,SFSsw=dataSW,GC=dataGC,CatInf=BoundInf,CatSup=BoundSup,lower=inf1,upper=sup1,method=optimAlgorithm,control=list(maxit=MAXIT,factr=FACTR,lmm=LMM))
	# Output of model 1
	...
	if (models[1] || models[2]){
	  ...
	  if(is.null(B_init2a)) B_init2a <- ml1$par[4]/f_init2a
		...
		ml2a <- optim(init2a,lnL2a,SFSneutral=dataNeutral,SFSws=dataWS,SFSsw=dataSW,GC=dataGC,CatInf=BoundInf,CatSup=BoundSup,lower=inf2a,upper=sup2a,method=optimAlgorithm,control=list(maxit=MAXIT,factr=FACTR,lmm=LMM))
		# Output of model 2a
	...
	if (models[2]){
		...
		ml2b <- optim(init2b,lnL2b,SFSneutral=dataNeutral,SFSws=dataWS,SFSsw=dataSW,GC=dataGC,CatInf=BoundInf,CatSup=BoundSup,lower=inf2b,upper=sup2b,method=optimAlgorithm,control=list(maxit=MAXIT,factr=FACTR,lmm=LMM))
		# Output of model 2b
...
optimizeBGCwith3errorDiffAl <- function(dataNeutral,dataWS,dataSW,dataGC,
                                  optimAlgorithm, # BEN: I have added this argument to use to change the methods used by the optim() function
                                  BoundInf=c(1:length(dataNeutral)),BoundSup=BoundInf,models=c(FALSE,FALSE),
                                  MAXIT=1000,FACTR=10^7,LMM=20,BMAX=100,
                                  verbose=0,PrintMessage=TRUE,PrintInit=FALSE,
  ...
  ml0 <- optim(init0,lnL0err3,SFSneutral=dataNeutral,SFSws=dataWS,SFSsw=dataSW,GC=dataGC,CatInf=BoundInf,CatSup=BoundSup,lower=inf0,upper=sup0,method=optimAlgorithm,control=list(maxit=MAXIT,factr=FACTR,lmm=LMM))
  warning <- FALSE
  # Output of model 0
  if(ml0$convergence==0) info <- "OK" else {info <- ml0$message; warning <- TRUE}
  if(verbose==0){
  ...
  ml1 <- optim(init1,lnL1err3,SFSneutral=dataNeutral,SFSws=dataWS,SFSsw=dataSW,GC=dataGC,CatInf=BoundInf,CatSup=BoundSup,lower=inf1,upper=sup1,method=optimAlgorithm,control=list(maxit=MAXIT,factr=FACTR,lmm=LMM))
  # Output of model 1
  if(ml1$convergence==0) info <- "OK" else {info <- ml1$message; warning <- TRUE}
  if(verbose==0){
    out1 <- list(lnL=-ml1$value,B=ml1$par[4],convergence=info)
  ...
  if (models[1] || models[2]){
    ...
    ml2a <- optim(init2a,lnL2aerr3,SFSneutral=dataNeutral,SFSws=dataWS,SFSsw=dataSW,GC=dataGC,CatInf=BoundInf,CatSup=BoundSup,lower=inf2a,upper=sup2a,method=optimAlgorithm,control=list(maxit=MAXIT,factr=FACTR,lmm=LMM))
    # Output of model 2a
    if(ml2a$convergence==0) info <- "OK" else {info <- ml2a$message; warning <- TRUE}
    if(verbose==0){
      out2a <- list(lnL=-ml2a$value,B=ml2a$par[4],f=ml2a$par[5],convergence=info)
  ...
  if (models[2]){
    ...
    ml2b <- optim(init2b,lnL2berr3,SFSneutral=dataNeutral,SFSws=dataWS,SFSsw=dataSW,GC=dataGC,CatInf=BoundInf,CatSup=BoundSup,lower=inf2b,upper=sup2b,method=optimAlgorithm,control=list(maxit=MAXIT,factr=FACTR,lmm=LMM))
    # Output of model 2b
    if(ml2b$convergence==0) info <- "OK" else {info <- ml2b$message; warning <- TRUE}
    if(verbose==0){
      out2b <- list(lnL=-ml2b$value,B0=ml2b$par[4],B1=ml2b$par[5],f=ml2b$par[6],convergence=info)
