...
myOutputFiles.mel.bs <- lapply(list.dirs(path = '../PAML/no_poly/bootstraps/mel/outputfiles')[2:6],
                               FUN = function(mydir){
                                 list.files(path = mydir,
                                            pattern = 'output', full.names = T)
                               })

myOutputFiles.sim.bs <- lapply(list.dirs(path = '../PAML/no_poly/bootstraps/sim/outputfiles')[2:6],
                               FUN = function(mydir){
                                 list.files(path = mydir,
                                            pattern = 'output', full.names = T)
                               })

myOutputFiles.mean.bs <- lapply(list.dirs(path = '../PAML/no_poly/bootstraps/mean/outputfiles')[2:6],
                                FUN = function(mydir){
                                  list.files(path = mydir,
                                             pattern = 'output', full.names = T)
                                })

myOutputFiles.diff.bs <- lapply(list.dirs(path = '../PAML/no_poly/bootstraps/diff/outputfiles')[2:6],
                                FUN = function(mydir){
                                  list.files(path = mydir,
                                             pattern = 'output', full.names = T)
                                })
...
save(list = ls(pattern = 'results'),
     file = '../data/PAML_bootstraps_noPoly.RData')
