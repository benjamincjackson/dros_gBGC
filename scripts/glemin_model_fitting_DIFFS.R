...
# carry out the glemin gamma model fitting and comparison 
...
GCcontent.Dsim.MD.SI.A.concat.diff.5GCbins <- sapply(
...
GCcontent.Dmel.ZI69.SI.A.concat.diff.5GCbins <- sapply(
...
# 
results_MD.SI.A.5GCbins.diff_no_error <- vector('list', length = 5)
system.time(
  for(i in 1:5){
    results_MD.SI.A.5GCbins.diff_no_error[[i]] <- get_test_results_no_error(SFS_Neu = N_to_N_ALL_SFSes.MD.SI.A.diff.5GCbins[[i]][2:21],
                                                                            SFS_S_2_W = GC_to_AT_ALL_SFSes.MD.SI.A.diff.5GCbins[[i]][2:21],
                                                                            SFS_W_2_S = AT_to_GC_ALL_SFSes.MD.SI.A.diff.5GCbins[[i]][2:21],
                                                                            GC = GCcontent.Dsim.MD.SI.A.concat.diff.5GCbins[i],
...
results_ZI69.SI.A.5GCbins.diff_no_error <- vector('list', length = 5)
system.time(
  for(i in 1:5){
    results_ZI69.SI.A.5GCbins.diff_no_error[[i]] <- get_test_results_no_error(SFS_Neu = N_to_N_ALL_SFSes.ZI69.SI.A.diff.5GCbins[[i]][2:69],
                                                                              SFS_S_2_W = GC_to_AT_ALL_SFSes.ZI69.SI.A.diff.5GCbins[[i]][2:69],
                                                                              SFS_W_2_S = AT_to_GC_ALL_SFSes.ZI69.SI.A.diff.5GCbins[[i]][2:69],
                                                                              GC = GCcontent.Dmel.ZI69.SI.A.concat.diff.5GCbins[i],
                                                                              algorithmToUse = "Nelder-Mead",
                                                                              maxit=1000)
  }
)
...
lapply(results_MD.SI.A.5GCbins.diff_no_error, FUN = function(x){x$conv_table})
lapply(results_ZI69.SI.A.5GCbins.diff_no_error, FUN = function(x){x$conv_table})
...
results_MD.SI.A.5GCbins.diff_error <- vector('list', length = 5)
system.time(
  for(i in 1:5){
    # this is just to make the arguments list in the next part less long:
    x <- results_MD.SI.A.5GCbins.diff_no_error[[i]]$without_error$model0
    y <- results_MD.SI.A.5GCbins.diff_no_error[[i]]$without_error$model1
    
    results_MD.SI.A.5GCbins.diff_error[[i]] <- get_test_results_error(SFS_Neu = N_to_N_ALL_SFSes.MD.SI.A.diff.5GCbins[[i]][2:21],
                                                                      SFS_S_2_W = GC_to_AT_ALL_SFSes.MD.SI.A.diff.5GCbins[[i]][2:21],
                                                                      SFS_W_2_S = AT_to_GC_ALL_SFSes.MD.SI.A.diff.5GCbins[[i]][2:21],
                                                                      GC = GCcontent.Dsim.MD.SI.A.concat.diff.5GCbins[i],
                                                                      algorithmToUse = "Nelder-Mead",
...
results_ZI69.SI.A.5GCbins.diff_error <- vector('list', length = 5)
system.time(
  for(i in 1:5){
    # this is just to make the arguments list in the next part less long:
    x <- results_ZI69.SI.A.5GCbins.diff_no_error[[i]]$without_error$model0
    y <- results_ZI69.SI.A.5GCbins.diff_no_error[[i]]$without_error$model1
    
    results_ZI69.SI.A.5GCbins.diff_error[[i]] <- get_test_results_error(SFS_Neu = N_to_N_ALL_SFSes.ZI69.SI.A.diff.5GCbins[[i]][2:69],
                                                                        SFS_S_2_W = GC_to_AT_ALL_SFSes.ZI69.SI.A.diff.5GCbins[[i]][2:69],
                                                                        SFS_W_2_S = AT_to_GC_ALL_SFSes.ZI69.SI.A.diff.5GCbins[[i]][2:69],
                                                                        GC = GCcontent.Dmel.ZI69.SI.A.concat.diff.5GCbins[i],
                                                                        algorithmToUse = "Nelder-Mead",
...
# check for convergence:
lapply(results_MD.SI.A.5GCbins.diff_error, FUN = function(x){x$conv_table})
lapply(results_ZI69.SI.A.5GCbins.diff_error, FUN = function(x){x$conv_table}) #bin 2, M0 is wrong

# so save the results
save(results_MD.SI.A.5GCbins.diff_no_error,
     results_ZI69.SI.A.5GCbins.diff_no_error,
     results_MD.SI.A.5GCbins.diff_error,
     results_ZI69.SI.A.5GCbins.diff_error,
     file = '../data/glemin_results_DIFFS.RData')
