...
# the gff info:
gff = pysam.TabixFile("/home/ben/data/brian_faster_X/Dsim_variant_calling/annotation/gff/dsim-all-r2.02.uniq.gff.cleaned.gz")

# NB probably need to exclude NR regions if possible at some point, as I did for the D melanogaster data.

# initiate an empty list to fill with short intron info

i=0
j = 0
with gzip.open("/home/ben/data/brian_faster_X/Dsim_variant_calling/annotation/flybase_other/dsim-all-intron-r2.02.fasta.gz", 'rt') as f_in:
    for l in f_in:
        ...
        # the name of the intron is the first item:
        name = line.split(' ', 1)[0]
        ...
        try:
            ...
            if safeintron == True:
                ...
                # Or write a gff file line with SI coordinates?
